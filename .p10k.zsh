# Generated by Powerlevel10k configuration wizard on 2023-07-05 at 13:15 CEST.
...
() {
  ...
  if [[ ${HOST} == "defra1rnd0002" ]]; then
    typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='babo'
  else
  ...
  function prompt_bitbake() {
    if [[ ${BITBAKE_PANE} == "bitbake" ]]; then
      p10k segment -f 248 -t "${BITBAKE_PANE}"
    fi
    if [[ ${HOST} == "bitbake" ]]; then
      p10k segment -f 248 -t "bitbake-docker"
    fi
  }
